...
from language import t, ui, value_label, get_lang
...
def render_auth_box() -> bool:
    ...
    if not get_configured():
        st.warning(ui("A camada social ainda não está configurada. Adicione SUPABASE_URL e SUPABASE_ANON_KEY nos Secrets do Streamlit."))
        with st.expander(ui("O que falta configurar?")):
            st.write(ui("Crie um projeto no Supabase, rode o SQL das tabelas sociais e adicione as chaves no Streamlit Secrets."))
        return False
    ...
    if user:
        ...
        st.sidebar.success(f"{ui('Logado como')} {name}")
        if st.sidebar.button(ui("Sair da conta")):
            try:
                sb.auth.sign_out()
            except Exception:
                pass
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader(ui("Conta social"))
    mode = st.sidebar.radio(ui("Acesso"), ["Entrar", "Criar conta"], format_func=ui, horizontal=True)
    email = st.sidebar.text_input(ui("Email"), key="social_email")
    password = st.sidebar.text_input(ui("Senha"), type="password", key="social_password")

    if mode == "Entrar":
        if st.sidebar.button(ui("Entrar")):
            try:
                ...
                st.sidebar.error(f"{ui('Erro ao entrar')}: {e}")
    else:
        username = st.sidebar.text_input(ui("Username"), key="signup_username")
        full_name = st.sidebar.text_input(ui("Nome"), key="signup_fullname")
        if st.sidebar.button(ui("Criar conta")):
            try:
                ...
                if user_obj and session_obj:
                    ...
                    st.sidebar.info(ui("Conta criada. Confirme seu email e depois faça login."))
                else:
                    st.sidebar.warning(ui("Não foi possível criar a conta."))
            except Exception as e:
                st.sidebar.error(f"{ui('Erro ao criar conta')}: {e}")

    return False


def render_profile_tab(sb: Client) -> None:
    st.subheader(ui("Meu perfil"))
    user_id = current_user_id()
    profile = get_profile(sb, user_id) or {}

    with st.form("edit_profile"):
        username = st.text_input(ui("Username"), value=profile.get("username") or "")
        full_name = st.text_input(ui("Nome completo"), value=profile.get("full_name") or "")
        institution = st.text_input(ui("Instituição"), value=profile.get("institution") or "")
        interests = st.text_area(ui("Interesses"), value=profile.get("interests") or "")
        bio = st.text_area(ui("Bio"), value=profile.get("bio") or "")
        avatar_url = st.text_input(ui("URL do avatar/foto"), value=profile.get("avatar_url") or "")
        submitted = st.form_submit_button(ui("Salvar perfil"))
    ...
    if submitted:
        try:
            ...
            st.success(ui("Perfil atualizado."))
            st.rerun()
        except Exception as e:
            st.error(f"{ui('Erro ao salvar perfil')}: {e}")

    st.markdown(ui("### Encontrar pesquisadores"))
    query = st.text_input(ui("Buscar por username, nome ou instituição"))
    if query:
        users = safe_data(
            sb.table("profiles")
        ...
        for p in users:
            ...
            with col2:
                if st.button(ui("Seguir"), key=f"follow_{p['id']}"):
                    try:
                        sb.table("follows").upsert({"follower_id": user_id, "followee_id": p["id"]}).execute()
                        notify_user(sb, p["id"], "follow", f"@{profile.get('username')} " + ui("começou a seguir você."))
                        st.success(ui("Seguindo."))
                    except Exception as e:
                        st.error(f"{ui('Erro')}: {e}")

    st.markdown(ui("### Minhas estatísticas"))
    ...
    try:
        ...
        c1.metric(ui("Comentários"), comments)
        c2.metric(ui("Análises salvas"), saved)
        c3.metric(ui("Notícias salvas"), articles)
        c4.metric(ui("Seguidores"), followers)
    except Exception:
        st.info(ui("Estatísticas indisponíveis agora."))


def render_comments_tab(sb: Client, context: Dict[str, Any]) -> None:
    st.subheader(ui("Comunidade e comentários da análise"))
    analysis_key = context["analysis_key"]
    user_id = current_user_id()
    profile = get_profile(sb, user_id) or {}

    st.markdown(
        f"**{ui('Discussão atual')}:** {context['community']} · {context['country']} · {value_label(context['sentiment'])}"
    )

    with st.form("new_comment"):
        comment_text = st.text_area(ui("Escrever comentário público"))
        submitted = st.form_submit_button(ui("Publicar comentário"))
    if submitted and comment_text.strip():
        try:
            ...
            st.success(ui("Comentário publicado."))
            st.rerun()
        except Exception as e:
            st.error(f"{ui('Erro ao comentar')}: {e}")
    ...
    if not comments:
        st.info(ui("Ainda não há comentários nesta análise."))
    else:
        for c in comments:
            author = c.get("profiles") or {}
            st.markdown("---")
            st.markdown(f"**{author.get('full_name') or author.get('username') or ui('Usuário')}** · {c.get('created_at', '')[:16]}")
                    ...
                    if st.button(reaction, key=f"react_{c['id']}_{reaction}"):
                        try:
                            ...
                            if c["user_id"] != user_id:
                                notify_user(sb, c["user_id"], "reaction", f"{ui('Seu comentário recebeu reação')} {reaction}.")
                            st.rerun()
                        except Exception as e:
                            st.error(f"{ui('Erro')}: {e}")

    st.markdown(ui("### Feed geral da comunidade"))
    feed = safe_data(
        sb.table("analysis_comments")
    ...
    for item in feed:
        ...
        st.markdown(
            f"- **{author.get('full_name') or author.get('username') or ui('Usuário')}** {ui('em')} "
            f"*{item.get('community')} · {item.get('destination_country')}*: {item.get('comment_text')}"
        )

    st.markdown(ui("### Mapa de vozes"))
    if feed:
        feed_df = pd.DataFrame(feed)
        voice_df = feed_df.groupby("destination_country").size().reset_index(name="comentários")
        fig = px.bar(voice_df, x="destination_country", y="comentários", title=ui("Comentários por país de destino"), labels={"destination_country": ui("País de destino"), "comentários": ui("comentários")})
        st.plotly_chart(fig, use_container_width=True)


def render_save_collections_tab(sb: Client, context: Dict[str, Any], filtered: pd.DataFrame) -> None:
    st.subheader(ui("Salvar análises, notícias e coleções"))
    user_id = current_user_id()

    c1, c2 = st.columns(2)
    with c1:
        note = st.text_area(ui("Nota para salvar esta análise"))
        if st.button(ui("Salvar análise atual")):
            try:
                ...
                st.success(ui("Análise salva."))
            except Exception as e:
                st.error(f"{ui('Erro')}: {e}")

    with c2:
        with st.form("new_collection"):
            title = st.text_input(ui("Nome da coleção"))
            description = st.text_area(ui("Descrição"))
            is_public = st.checkbox(ui("Coleção pública"))
            submitted = st.form_submit_button(ui("Criar coleção"))
        if submitted and title:
            try:
                ...
                st.success(ui("Coleção criada."))
                st.rerun()
            except Exception as e:
                st.error(f"{ui('Erro')}: {e}")

    st.markdown(f"### {ui('Salvar notícia da seleção')}")
    if filtered.empty:
        st.info(ui("Nenhuma notícia filtrada para salvar."))
    else:
        ...
        selected_label = st.selectbox(ui("Escolha uma notícia"), options["label"].tolist())
        selected = options[options["label"] == selected_label].iloc[0]
        article_note = st.text_area(ui("Nota privada sobre a notícia"))
        if st.button(ui("Salvar notícia")):
            try:
                ...
                st.success(ui("Notícia salva."))
            except Exception as e:
                st.error(f"{ui('Erro')}: {e}")

    st.markdown(ui("### Minhas análises e notícias salvas"))
...
def render_framing_debate_tab(sb: Client, context: Dict[str, Any], filtered: pd.DataFrame, story: str) -> None:
    st.subheader(ui("Framing colaborativo, debate e revisão humana da IA"))
    user_id = current_user_id()

    st.markdown(ui("### Framing Battle: algoritmo × humanos"))
    if filtered.empty:
        st.info(ui("Nenhuma notícia filtrada para votar framing."))
    else:
        options = filtered.sort_values("date", ascending=False).head(50).copy()
        options["label"] = options.apply(lambda r: f"{int(r['article_id'])} · {ui('automático')}: {value_label(r.get('frame', 'n/d'))} · {r['url'][:60]}", axis=1)
        selected_label = st.selectbox(ui("Notícia para classificar"), options["label"].tolist(), key="framing_article")
        selected = options[options["label"] == selected_label].iloc[0]
        frame = st.selectbox(ui("Qual é o framing dominante?"), SOCIAL_FRAMES, format_func=value_label)
        note = st.text_area(ui("Justificativa opcional"))
        if st.button(ui("Votar framing")):
            try:
        ...
        if votes:
            ...
            fig = px.bar(vote_df, x="frame", y="votos", title=ui("Votos humanos de framing"))
            st.plotly_chart(fig, use_container_width=True)

    st.markdown(ui("### Debate Mode"))
    if get_lang() == "en":
        default_statement = f"The coverage about {context['community']} in {context['country']} is invisible relative to its demographic weight."
    else:
    ...
    statement = st.text_input(ui("Afirmação para debate"), value=default_statement)
    if st.button(ui("Criar debate desta análise")):
        try:
    ...
    for d in debates:
        with st.expander(d["statement"]):
            position = st.radio(ui("Sua posição"), ["Concordo", "Discordo", "Depende"], format_func=ui, key=f"pos_{d['id']}", horizontal=True)
            justification = st.text_area(ui("Justificativa"), key=f"just_{d['id']}")
            if st.button(ui("Votar no debate"), key=f"vote_debate_{d['id']}"):
                try:
    ...
    st.markdown(ui("### Revisão humana de IA / Story Mode"))
    rating = st.radio(ui("A IA interpretou bem?"), ["Sim", "Parcialmente", "Não"], format_func=ui, horizontal=True)
    feedback = st.text_area(ui("Comentário sobre a interpretação da IA"), value="")
    if st.button(ui("Salvar avaliação da IA")):
        try:
            ...
            st.success(ui("Avaliação salva."))
        except Exception as e:
            st.error(f"{ui('Erro')}: {e}")


def render_messages_tab(sb: Client) -> None:
    st.subheader(ui("Mensagens diretas"))
    user_id = current_user_id()

    st.markdown(ui("### Nova conversa"))
    username = st.text_input(ui("Username da pessoa"))
    if st.button(ui("Criar conversa")) and username:
        target = safe_data(sb.table("profiles").select("id, username").eq("username", username).limit(1).execute())
        if not target:
            st.error(ui("Usuário não encontrado."))
        else:
            ...
            try:
                ...
                notify_user(sb, target_id, "dm", ui("Você recebeu uma nova conversa."))
                st.success(ui("Conversa criada."))
                st.rerun()
            except Exception as e:
                st.error(f"{ui('Erro')}: {e}")
    ...
    if not conv_ids:
        st.info(ui("Você ainda não tem conversas."))
        return

    conv_id = st.selectbox(ui("Escolha uma conversa"), conv_ids)
    messages = safe_data(
    ...
    for m in messages:
        ...
        st.markdown(f"**{author.get('full_name') or author.get('username') or ui('Usuário')}:** {m['message_text']}")
        st.caption(m.get("created_at", ""))

    with st.form("send_dm"):
        msg = st.text_area(ui("Mensagem"))
        submitted = st.form_submit_button(ui("Enviar"))
        ...
        try:
            ...
            for m in members:
                if m["user_id"] != user_id:
                    notify_user(sb, m["user_id"], "dm", ui("Nova mensagem direta recebida."))
            st.rerun()
        except Exception as e:
            st.error(f"{ui('Erro ao enviar')}: {e}")


def render_circles_reports_tab(sb: Client, context: Dict[str, Any]) -> None:
    st.subheader(ui("Research Circles e relatórios colaborativos"))
    user_id = current_user_id()

    st.markdown(ui("### Criar círculo de pesquisa"))
    with st.form("new_circle"):
        title = st.text_input(ui("Título do círculo"))
        theme = st.text_input(ui("Tema"), value="Invisibilidade midiática")
        desc = st.text_area(ui("Descrição"))
        submitted = st.form_submit_button(ui("Criar círculo"))
    if submitted and title:
        try:
    ...
    if circles:
        ...
        chosen = st.selectbox(ui("Círculos disponíveis"), circle_labels)
        circle_id = int(chosen.split(" · ")[0])
        if st.button(ui("Entrar neste círculo")):
            try:
                sb.table("circle_members").upsert({"circle_id": circle_id, "user_id": user_id}).execute()
                st.success(ui("Você entrou no círculo."))
            except Exception as e:
                st.error(f"{ui('Erro')}: {e}")

        with st.form("circle_post"):
            post_text = st.text_area(ui("Postar no círculo"))
            submitted = st.form_submit_button(ui("Publicar no círculo"))
        ...
        for p in posts:
            ...
            st.markdown(f"**{author.get('full_name') or author.get('username') or ui('Usuário')}:** {p['post_text']}")
            st.caption(p.get("created_at", ""))

    st.markdown("---")
    st.markdown(ui("### Collab Report"))
    with st.form("new_report"):
        report_title = st.text_input(ui("Título do relatório colaborativo"), value=f"Relatório — {context['community']} em {context['country']}")
        report_body = st.text_area(ui("Texto inicial"))
        submitted = st.form_submit_button(ui("Criar relatório"))
    if submitted and report_title:
        try:
            ...
            st.success(ui("Relatório criado."))
            st.rerun()
        except Exception as e:
            st.error(f"{ui('Erro')}: {e}")
    ...
    for r in reports:
        with st.expander(r["title"]):
            st.write(r.get("body") or ui("Sem texto ainda."))
            collaborator_username = st.text_input(ui("Adicionar colaborador por username"), key=f"collab_user_{r['id']}")
            if st.button(ui("Adicionar colaborador"), key=f"add_collab_{r['id']}") and collaborator_username:
                ...
                if target:
                    try:
                        ...
                        notify_user(sb, target[0]["id"], "report", f"{ui('Você foi adicionado ao relatório')}: {r['title']}")
                        st.success(ui("Colaborador adicionado."))
                    except Exception as e:
                        st.error(f"{ui('Erro')}: {e}")
                else:
                    st.error(ui("Usuário não encontrado."))
...
def render_diary_notifications_tab(sb: Client, context: Dict[str, Any]) -> None:
    st.subheader(ui("Diário da diáspora, notificações e ranking"))
    user_id = current_user_id()

    st.markdown(ui("### Diário da diáspora"))
    with st.form("diary"):
        title = st.text_input(ui("Título da reflexão"))
        body = st.text_area(ui("Reflexão qualitativa"))
        public = st.checkbox(ui("Tornar reflexão pública"))
        submitted = st.form_submit_button(ui("Salvar reflexão"))
    ...
    for entry in entries:
        with st.expander(entry.get("title") or ui("Reflexão")):
            author = entry.get("profiles") or {}
            st.caption(f"{author.get('full_name') or author.get('username') or ui('Usuário')} · {entry.get('community')} · {entry.get('destination_country')}")
            st.write(entry.get("body"))

    st.markdown(ui("### Notificações"))
    ...
    if notes:
        ...
        if st.button(ui("Marcar todas como lidas")):
            sb.table("notifications").update({"read": True}).eq("user_id", user_id).execute()
            st.rerun()
    else:
        st.info(ui("Sem notificações."))

    st.markdown(ui("### Ranking de comentários úteis"))
    reactions = safe_data(
    ...
    if reactions:
        ...
        _rank_cols = {"comentário": ui("comentário"), "autor": ui("autor"), "contexto": ui("contexto"), "reações": ui("reações")}
        st.dataframe(rank.sort_values("reações", ascending=False).head(10).rename(columns=_rank_cols), use_container_width=True, hide_index=True)
    else:
        st.info(ui("Ainda não há reações suficientes para ranking."))


def render_case_of_week(articles: pd.DataFrame) -> None:
    st.markdown(ui("### Invisible Case of the Week"))
    if articles is None or articles.empty:
        st.info(ui("Sem dados para caso da semana."))
        return
    try:
        case = (
        ...
        if not case.empty:
            ...
            if get_lang() == "en":
                st.warning(
                    f"Suggested critical case: {r['community']} in {r['destination_country']} — "
                    f"{int(r['noticias'])} news items, average tone {r['tom_medio']:.2f}."
                )
            else:
                st.warning(
        ...
        st.info(ui("Caso da semana indisponível."))
...
def render_social_layer(
    ...
    st.markdown("---")
    st.header(ui("🌐 Camada colaborativa"))
    st.caption(ui("Contas, perfis, comentários, mensagens, coleções, debates e análise humana colaborativa."))

    logged_in = render_auth_box()
    if not logged_in:
        st.info(ui("Entre ou crie uma conta para usar a camada colaborativa."))
        return
    ...
    with top_left:
        st.markdown(ui("### 🌐 Camada colaborativa"))
