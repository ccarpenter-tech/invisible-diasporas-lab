...
def risk_label(value, all_values):
    ...
    if value >= q90:
        return value_label("Invisibilidade crítica"), "risk-critical"
    elif value >= q75:
        return value_label("Alta invisibilidade"), "risk-high"
    elif value >= q50:
        return value_label("Invisibilidade moderada"), "risk-medium"
    else:
        return value_label("Baixa invisibilidade"), "risk-low"
...
def pandemic_phase(year):
    if year <= 2019:
        return value_label("Pré-pandemia")
    elif year in [2020, 2021]:
        return value_label("Pandemia")
    else:
        return value_label("Pós-pandemia")


def narrative_risk_score(filtered, pair_row, global_pair_inv):
    if filtered.empty or pair_row is None:
        return 0, value_label("Risco não calculável")
    ...
    if score >= 75:
        label = value_label("Risco narrativo crítico")
    elif score >= 55:
        label = value_label("Risco narrativo alto")
    elif score >= 35:
        label = value_label("Risco narrativo moderado")
    else:
        label = value_label("Risco narrativo baixo")
...
def generate_insights(filtered, pair_row, selected_community, selected_country, pair_inv_global):
    ...
    return [ui(i) for i in insights]


def story_mode_text(selected_community, selected_country, filtered, pair_row, risk_text):
    if filtered.empty:
        return ui(f"Não há cobertura suficiente para construir uma narrativa sobre {selected_community} em {selected_country}.")
...
risk_text = value_label("Risco não disponível")
risk_css = "risk-low"
