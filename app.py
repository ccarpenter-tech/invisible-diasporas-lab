...
from language import t, ui, value_label, get_lang, language_selector
...
def summarize_with_claude(rows, selected_community, selected_country):
    ...
    if not api_key:
        return ui("Resumo por IA indisponível: adicione ANTHROPIC_API_KEY nos Secrets do Streamlit.")
...
"""
        ...
        return ui("Não foi possível gerar o resumo por IA") + ". " + ui("Erro") + f": {e}"
...
def risk_label(value, all_values):
    ...
    if value >= q90:
        return ui("Invisibilidade crítica"), "risk-critical"
    elif value >= q75:
        return ui("Alta invisibilidade"), "risk-high"
    elif value >= q50:
        return ui("Invisibilidade moderada"), "risk-medium"
    else:
        return ui("Baixa invisibilidade"), "risk-low"


def tone_explanation(avg_tone):
    if pd.isna(avg_tone):
        return ui("Não há tom médio disponível para esta seleção.")
    if avg_tone <= -2:
        return ui("A cobertura apresenta tom médio negativo.")
    elif avg_tone >= 2:
        return ui("A cobertura apresenta tom médio positivo.")
    else:
        return ui("A cobertura apresenta tom médio próximo de neutro.")
...
def narrative_risk_score(filtered, pair_row, global_pair_inv):
    ...
    if score >= 75:
        label = ui("Risco narrativo crítico")
    elif score >= 55:
        label = ui("Risco narrativo alto")
    elif score >= 35:
        label = ui("Risco narrativo moderado")
    else:
        label = ui("Risco narrativo baixo")
    ...
    return pd.DataFrame(
        {
            "dimensão": [
                ...
                ui("Tom positivo"),
                ui("Invisibilidade"),
                ui("Diversidade de fontes"),
                ui("Diversidade de enquadramentos"),
                ui("Presença temporal")
            ],
            "valor": [
                coverage_score,
                tone_score,
...
def generate_insights(filtered, pair_row, selected_community, selected_country, pair_inv_global):
    ...
    if filtered.empty:
        return [ui("Não há notícias disponíveis para gerar insights com os filtros atuais.")]
    ...
    if en:
        insights.append(f"The selection contains {total_news} news items about {selected_community} in {selected_country}.")
    else:
        insights.append(f"A seleção contém {total_news} notícias sobre {selected_community} em {selected_country}.")

    insights.append(tone_explanation(avg_tone))

    if top_frame:
        if en:
            insights.append(f"The dominant framing identified is: {value_label(top_frame)}.")
        else:
            insights.append(f"O enquadramento predominante identificado é: {value_label(top_frame)}.")

    if pair_row is not None:
        label, _ = risk_label(pair_row["invisibility_index_proxy"], pair_inv_global["invisibility_index_proxy"])
        if en:
            insights.append(f"The invisibility diagnosis is: {label}.")
        else:
            insights.append(f"O diagnóstico de invisibilidade é: {label}.")

    if diversity < 0.35:
        insights.append(ui("A diversidade de fontes é baixa: poucas fontes concentram a cobertura."))
    elif diversity > 0.70:
        insights.append(ui("A cobertura apresenta alta diversidade de fontes jornalísticas."))

    years = sorted(filtered["year"].dropna().unique().tolist())
    if len(years) > 1:
        if en:
            insights.append(f"Coverage appears distributed between {int(min(years))} and {int(max(years))}.")
        else:
...
def story_mode_text(selected_community, selected_country, filtered, pair_row, risk_text):
    ...
    if filtered.empty:
        if en:
            return f"There is not enough coverage to build a narrative about {selected_community} in {selected_country}."
        return f"Não há cobertura suficiente para construir uma narrativa sobre {selected_community} em {selected_country}."

    avg_tone = filtered["tone"].mean()
    dominant_frame = value_label(filtered["frame"].value_counts().idxmax())
    total_news = len(filtered)
    ...
    if en:
        story = (
            f"The {selected_community} community in {selected_country} appears in the corpus through "
            f"{total_news} filtered news items. The dominant framing is '{dominant_frame}', "
            f"which suggests that coverage tends to organize this community mainly around that topic. "
            f"The average tone of the coverage is {avg_tone:.2f}, so {tone_explanation(avg_tone).lower()} "
        )
    else:
        story = (
    ...
    if invisibility is not None:
        if en:
            story += (
                f"The invisibility proxy index is {invisibility:.2f}, classified as {risk_text.lower()}. "
                f"This indicates that media visibility must be interpreted in relation to demographic weight "
                f"and not only to the absolute number of news items. "
            )
        else:
            story += (
    ...
    if en:
        story += (
            "In communication terms, the result helps observe whether the media treats the diaspora as a complex social subject "
            "or whether it appears only in specific contexts, such as labor, crisis, security, economy or culture."
        )
    else:
        story += (
...
def build_report(selected_community, selected_country, selected_sentiment, filtered, pair_row, insights, narrative_score, narrative_label, story):
    ...
    en = get_lang() == "en"
    na = "not available" if en else "não disponível"
    ...
    if pair_row is not None:
        ...
        invisibility = na
        coverage = total_news
        demographic_proxy = na

    avg_tone_text = f"{avg_tone:.2f}" if not pd.isna(avg_tone) else na
    dominant_frame = value_label(dominant_frame)
...
Filtered sentiment: {value_label(selected_sentiment)}

## Key metrics
News found: {total_news}
Coverage used in the index: {coverage}
Average tone: {avg_tone_text}
Dominant framing: {dominant_frame}
Source diversity: {diversity:.2f}
Demographic proxy: {demographic_proxy}
Invisibility index: {invisibility}
Narrative Risk Score: {narrative_score}/100
Narrative risk classification: {narrative_label}
...
Qualitatively investigate the most relevant news to understand whether the community is represented as a social subject,
as a workforce, as a public problem, as an economic actor or as a cultural presence.

## Methodological note
The community x destination index is a comparative proxy.
It combines the global size of the origin diaspora with the migratory weight of the destination country.
It does not represent an exact bilateral count of migrants by origin and destination.
"""
    else:
        report = f"""
# Executive Briefing — Invisible Diasporas Lab
...
Sentimento filtrado: {value_label(selected_sentiment)}
...
def recommend_readings(filtered, all_articles, selected_community, selected_country):
    ...
    if avg_tone < 0:
        ...
        recommendations[ui("Leia também: notícias com tom mais positivo")] = opposite
    else:
        opposite = all_articles[
            (all_articles["community"] == selected_community) &
            (all_articles["tone"] < 0)
        ].head(5)
        recommendations[ui("Leia também: notícias com tom mais negativo")] = opposite
    ...
    recommendations[ui("Mesma comunidade em outros países")] = same_community_other_country
    ...
    recommendations[ui("Outras comunidades no mesmo país")] = same_country_other_community
...
def claude_story(rows, selected_community, selected_country, story):
    ...
    if not api_key:
        return ui("Story Mode com IA indisponível: adicione ANTHROPIC_API_KEY nos Secrets do Streamlit.")
...
"""
        ...
        return ui("Não foi possível gerar a narrativa por IA") + ". " + ui("Erro") + f": {e}"
...
risk_text = ui("Risco não disponível")
risk_css = "risk-low"
...
if presentation_mode:
    st.subheader(ui("🎤 Modo apresentação"))
    ...
    col1.metric(ui("Notícias"), f"{total_news:,}".replace(",", "."))
    col2.metric(ui("Tom médio"), f"{avg_tone:.2f}" if total_news else "—")
    col3.metric(ui("Invisibilidade"), risk_text)
    col4.metric(ui("Narrative Risk"), f"{narrative_score}/100")

    st.markdown(
        f"""
        <div class="{risk_css}">
        {ui('Diagnóstico')}: {risk_text} · {narrative_label}
        </div>
        """,
        unsafe_allow_html=True
    )

    st.markdown(ui("### Principais achados"))
    for insight in insights[:4]:
        st.write("• " + insight)

    st.markdown(ui("### Story Mode"))
    st.write(story)
...
with tab1:
    ...
    col3.metric(t("dominant_frame"), value_label(dominant_frame))
    ...
    if not filtered.empty:
        ...
        with c1:
            ...
            fig_sentiment = px.pie(
                ...
                names="sentiment",
                values="count",
                title=ui("Distribuição do sentimento"),
                hole=0.45
            )
        ...
        with c2:
            ...
            fig_frame = px.bar(
                ...
                y="frame",
                orientation="h",
                title=ui("Enquadramentos predominantes")
            )
        ...
        if get_lang() == "en":
            finding_1 = f"The filtered coverage gathers {total_news} news items about {selected_community} in {selected_country}."
            finding_2 = f"The dominant framing is '{value_label(dominant_frame)}'."
        else:
            finding_1 = f"A cobertura filtrada reúne {total_news} notícias sobre {selected_community} em {selected_country}."
            finding_2 = f"O enquadramento predominante é '{value_label(dominant_frame)}'."
        finding_3 = tone_explanation(avg_tone)

        f1, f2, f3 = st.columns(3)

        with f1:
            st.markdown(f"<div class='finding-card'><b>{ui('Achado')} 1</b><br>{finding_1}</div>", unsafe_allow_html=True)

        with f2:
            st.markdown(f"<div class='finding-card'><b>{ui('Achado')} 2</b><br>{finding_2}</div>", unsafe_allow_html=True)

        with f3:
            st.markdown(f"<div class='finding-card'><b>{ui('Achado')} 3</b><br>{finding_3}</div>", unsafe_allow_html=True)
        ...
        st.info(ui("Nenhuma notícia encontrada com esses filtros."))
...
with tab2:
    st.subheader(ui("Mapa crítico de invisibilidade"))

    if get_lang() == "en":
        st.markdown(
            """
            <div class="method-box">
            The map shows invisibility hotspots by destination country.
            Darker countries indicate higher relative invisibility.
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    if not country_inv_for_map.empty:
        fig_map = px.choropleth(
            ...
            title=f"{ui('Hotspots de invisibilidade')} — {selected_community}"
        )

        st.plotly_chart(fig_map, use_container_width=True)

        st.markdown(ui("### Top 5 hotspots críticos"))
        ...
        st.info(ui("Não há dados suficientes para o mapa crítico."))
...
with tab3:
    st.subheader(ui("Radar da cobertura e Narrative Risk Score"))
    ...
    if pair_row is not None:
        ...
        st.markdown(
            f"""
            <div class="{css_class}">
            {ui('Status de invisibilidade')}: {label}
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    if pair_row is not None:
        col1.metric(ui("Cobertura encontrada"), int(pair_row["coverage_count"]))
        col2.metric(ui("Índice de invisibilidade"), f"{pair_row['invisibility_index_proxy']:.2f}")
        col3.metric(ui("Proxy demográfico"), f"{pair_row['demographic_proxy']:,.0f}".replace(",", "."))

    st.markdown(ui("### Explique este resultado"))

    with st.expander(ui("Por que o índice pode ser alto?")):
        st.write(
            ui("O índice aumenta quando há uma combinação de presença demográfica relevante e pouca cobertura midiática encontrada no corpus.")
        )

    with st.expander(ui("Por que o risco narrativo pode ser alto?")):
        st.write(
            ui("O Narrative Risk Score combina invisibilidade, tom negativo, enquadramentos sensíveis e baixa diversidade de fontes.")
        )

    with st.expander(ui("Esse número é uma verdade absoluta?")):
        st.write(
            ui("Não. Ele é um indicador comparativo. Serve para orientar análise crítica, não para substituir leitura qualitativa das notícias.")
        )
...
with tab4:
    st.subheader(ui("Linha do tempo, pandemia e silêncio midiático"))
    ...
    if not filtered.empty:
        ...
        fig_volume = px.line(
            ...
            y="news_count",
            markers=True,
            title=f"{ui('Volume mensal')} — {selected_community} · {selected_country}"
        )
            ...
            timeline_events = pd.DataFrame(
                {
                    ...
                    "event": [
                        "Pre-pandemic",
                        "Covid-19 pandemic",
                        "Debates on anti-Asian racism",
                        "Reopening and migratory reorganization",
            ...
            timeline_events = pd.DataFrame(
                {
                    ...
                    "evento": [
                        ui("Pré-pandemia"),
                        ui("Pandemia de Covid-19"),
                        ui("Debates sobre racismo anti-asiático"),
                        ui("Reabertura e reorganização migratória"),
                        ui("Normalização pós-pandemia")
                    ]
                }
            )

        st.markdown(ui("### Eventos-chave para leitura da cobertura"))
        ...
        st.markdown(ui("### Antes, durante e depois da pandemia"))
        _phase_cols = {"fase": ui("Fase"), "noticias": ui("Notícias"), "tom_medio": ui("Tom médio"), "enquadramentos": ui("Enquadramentos")}
        st.dataframe(phase_summary.rename(columns=_phase_cols), use_container_width=True, hide_index=True)

        fig_phase = px.bar(
            phase_summary,
            x="fase",
            y="noticias",
            title=ui("Cobertura por fase histórica"),
            labels={"fase": ui("Fase"), "noticias": ui("Notícias")},
            text_auto=True
        )

        st.plotly_chart(fig_phase, use_container_width=True)

        st.markdown(ui("### Silence Detector"))

        silence_df = detect_silence_periods(filtered)

        if silence_df.empty:
            st.success(ui("Não foram detectados longos períodos de silêncio dentro do intervalo filtrado."))
        else:
            st.warning(ui("Foram encontrados períodos sem cobertura no intervalo analisado."))
            st.dataframe(silence_df.head(10), use_container_width=True, hide_index=True)

    else:
        st.info(ui("Nenhum dado disponível para a seleção."))
...
with tab5:
    st.subheader(ui("Fontes jornalísticas, diversidade e baseline"))
    ...
    if not filtered.empty:
        ...
        st.markdown(ui("### Top fontes jornalísticas"))
        ...
        fig_sources = px.bar(
            ...
            y="source",
            orientation="h",
            title=ui("Fontes que mais aparecem na seleção")
        )
        ...
        st.metric(ui("Media Diversity Index"), f"{diversity:.2f}")

        if diversity < 0.35:
            st.warning(ui("A cobertura é concentrada em poucas fontes."))
        elif diversity < 0.70:
            st.info(ui("A cobertura tem diversidade intermediária de fontes."))
        else:
            st.success(ui("A cobertura apresenta alta diversidade de fontes."))

        st.markdown(ui("### Comparação com baseline geral"))
        ...
        if get_lang() == "en":
            baseline_df = pd.DataFrame(
                {
                    "Indicator": [
                        "Selection average tone",
                        "Overall average tone",
                        "Selection news",
                        "Average news per pairing",
                        "Selection invisibility",
                        "Overall average invisibility"
                    ],
                    "Value": [
                        f"{selection_tone:.2f}",
                        f"{baseline_tone:.2f}",
                        len(filtered),
                        f"{baseline_news:.2f}",
                        f"{selection_inv:.2f}" if not pd.isna(selection_inv) else "n/a",
                        f"{baseline_inv:.2f}"
                    ]
                }
            )
        ...
        st.info(ui("Nenhum dado disponível para fontes e baseline."))
...
with tab6:
    ...
    if query:
        try:
            ...
            if subset.empty:
                st.warning(ui("Nenhuma notícia disponível com os filtros atuais."))
            else:
                ...
                if st.button(ui("Gerar Story Mode com IA")):
                    with st.spinner("Gerando narrativa..."):
                        ai_story = claude_story(results, selected_community, selected_country, story)

                    st.markdown(ui("### Narrativa por IA"))
                    st.write(ai_story)

        except Exception as e:
            st.error(f"{ui('Erro na busca semântica')}: {e}")
    else:
        st.info(ui("Digite uma consulta ou escolha uma pergunta sugerida."))

    st.markdown(ui("### Recomendações de leitura"))
    ...
    if recs:
        for title, rec_df in recs.items():
            with st.expander(title):
                if rec_df.empty:
                    st.write(ui("Nenhuma recomendação disponível."))
                else:
                    render_news_cards(rec_df, max_items=5)
    else:
        st.info(ui("Não há recomendações disponíveis para esta seleção."))
...
with tab7:
    st.subheader(ui("Comparações e rankings"))

    st.markdown(ui("### Comparar duas comunidades no mesmo país"))

    c1, c2, c3 = st.columns(3)

    with c1:
        comp_country = st.selectbox(ui("País para comparação"), countries, index=countries.index(selected_country))

    with c2:
        comp_comm_a = st.selectbox(ui("Comunidade A"), communities, index=communities.index(selected_community))

    with c3:
        default_b = 1 if len(communities) > 1 else 0
        comp_comm_b = st.selectbox(ui("Comunidade B"), communities, index=default_b)
    ...
    if not comp_df.empty:
        fig_comp = px.bar(
            ...
            title=f"{ui('Invisibilidade comparada em')} {comp_country}",
            text_auto=".2f"
        )
        st.plotly_chart(fig_comp, use_container_width=True)

    st.markdown(ui("### Comparar uma comunidade em dois países"))

    p1, p2, p3 = st.columns(3)

    with p1:
        country_comm = st.selectbox(ui("Comunidade para comparação entre países"), communities, index=communities.index(selected_community))

    with p2:
        country_a = st.selectbox(ui("País A"), countries, index=countries.index(selected_country))

    with p3:
        default_country_b = 1 if len(countries) > 1 else 0
        country_b = st.selectbox(ui("País B"), countries, index=default_country_b)
    ...
    if not comp_country_df.empty:
        fig_country = px.bar(
            ...
            y="invisibility_index_proxy",
            title=f"{ui('Invisibilidade de')} {country_comm} {ui('por país')}",
            text_auto=".2f"
        )
        st.plotly_chart(fig_country, use_container_width=True)

    st.markdown(ui("### Visibilidade × tom"))
    ...
    fig_scatter = px.scatter(
        ...
        color="community",
        hover_name="destination_country",
        title=ui("Visibilidade × tom médio"),
        labels={
            "coverage_count": ui("Volume de cobertura"),
            "avg_tone": ui("Tom médio")
        }
    )

    st.plotly_chart(fig_scatter, use_container_width=True)

    st.markdown(ui("### Ranking crítico"))
    ...
    _rank_disp = [ui(o) for o in _rank_opts]
    rank_choice = st.radio(
        ui("Ranking"),
        _rank_disp,
        horizontal=True
    )
    rank_option = _rank_opts[_rank_disp.index(rank_choice)]
...
with tab8:
    st.subheader(ui("Story Mode e relatórios"))
    ...
    st.markdown(ui("### Executive Briefing"))

    st.text_area(
        ui("Relatório gerado automaticamente"),
        value=report,
        height=460
    )

    st.download_button(
        ui("Baixar briefing em Markdown"),
        data=report.encode("utf-8"),
        file_name="executive_briefing_diaspora.md",
        mime="text/markdown"
    )

    st.markdown(ui("### Histórico de consultas nesta sessão"))

    if st.session_state["history"]:
        st.dataframe(pd.DataFrame(st.session_state["history"]), use_container_width=True, hide_index=True)
    else:
        st.info(ui("Nenhuma seleção foi salva no histórico ainda. Use o botão na barra lateral."))
...
with tab9:
    st.subheader(ui("Sala de aula, modo pesquisador, glossário e sobre"))

    if analysis_mode == t("classroom"):
        st.markdown(ui("### Perguntas para discussão em sala"))
    ...
    elif analysis_mode == t("researcher"):
        st.markdown(ui("### Modo pesquisador"))
            ...
            researcher_df = pd.DataFrame(
                {
                    ...
                    "Description": [
                        "News/journalistic metadata related to Asian communities",
                        "BigQuery/GDELT",
                        "UN DESA International Migrant Stock 2020",
                        "Average emotional tone of the news",
                        "Proxy between media coverage and demographic weight",
                        "Keyword-based classification into thematic categories",
                        "The UN DESA tables used are not a bilateral origin x destination matrix"
                    ]
                }
            )
        else:
        ...
        st.markdown(ui("### Glossário interativo"))

    with st.expander(ui("Diáspora")):
        st.write(ui("Comunidade que vive fora de seu território de origem, mantendo vínculos culturais, históricos, familiares ou simbólicos com esse lugar."))

    with st.expander(ui("Invisibilidade midiática")):
        st.write(ui("Distância entre a presença social/demográfica de um grupo e a atenção que ele recebe na cobertura jornalística."))

    with st.expander(ui("Framing / enquadramento")):
        st.write(ui("Forma como a mídia organiza narrativamente um tema, destacando certos aspectos e apagando outros."))

    with st.expander(ui("Tom médio")):
        st.write(ui("Indicador que resume a tonalidade emocional das notícias. Valores negativos sugerem cobertura mais negativa; valores positivos sugerem cobertura mais positiva."))

    with st.expander(ui("Proxy demográfico")):
        st.write(ui("Indicador aproximado usado quando não há dado bilateral exato. Neste app, combina o tamanho global da diáspora de origem com o peso migratório do país de destino."))

    with st.expander(ui("Busca semântica")):
        st.write(ui("Busca por significado, não apenas por palavra-chave. Ela usa embeddings multilíngues para encontrar notícias conceitualmente próximas da consulta."))

    st.markdown(ui("### Sobre o projeto"))

    if get_lang() == "en":
        st.markdown(
            """
            <div class="soft-box">
            <b>Final Project A2 — Programming / Digital Communication — FGV</b><br><br>
            Developed by Carolina Carpenter, Matheus Constantin and Gabriel Sampaio.<br><br>
            The goal is to analyze the Asian diaspora through the lens of global media,
            investigating coverage, tone, framing and media invisibility.
            The final version works as an interactive media-analysis observatory.
            </div>
            """,
            unsafe_allow_html=True
        )
...
try:
    ...
    st.info(ui("Camada social não instalada: adicione social_layer.py ao projeto."))
except Exception as e:
    st.warning(f"{ui('Camada social temporariamente indisponível')}: {e}")
