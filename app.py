...
def render_news_cards(df, max_items=6):
    ...
    for _, row in examples.iterrows():
        st.markdown(
            f"""
            <div class="news-card">
            <b>{row['community']} · {row['destination_country']} · {row['sentiment']}</b><br>
            Data: {row['date'].date()} · Fonte: {row['source']} · Tom: {row['tone']}<br>
            Enquadramento: {row.get('frame', 'não identificado')}<br>
            <a href="{row['url']}" target="_blank">Abrir notícia</a>
            </div>
            """,
            unsafe_allow_html=True
        )
...
selected_community = st.sidebar.selectbox(t("community_filter"), communities)
selected_country = st.sidebar.selectbox(t("country_filter"), countries)
selected_sentiment = st.sidebar.selectbox(t("sentiment_filter"), sentiments)
...
if st.sidebar.button("Salvar seleção no histórico"):
...
if presentation_mode:
    ...
    st.markdown("### Notícias de exemplo")
